...
public class TestDominators {
    ...
    public void testDominatorTree() {
        ...
        for (BasicBlock n: nodes) {
            if (expectedIdoms[(int)n.bid] == 0)
                Assertions.assertNull(n.idom);
            else
                Assertions.assertEquals(expectedIdoms[(int)n.bid], n.idom.bid);
        }
        // -1 means empty set
        long[] expectedDF = {0,-1,2,4,2,-1,4,8,4,8};
        for (BasicBlock n: nodes) {
            if (expectedDF[(int)n.bid] == -1) {
                Assertions.assertTrue(n.dominationFrontier.isEmpty());
            }
            else {
                Assertions.assertEquals(1,n.dominationFrontier.size());
            }
        }
    }
    ...
    public void testLoopNests() {
        ...
        Assertions.assertEquals(2, loopNests.get(0)._loopHead.bid);
        Assertions.assertEquals(2, loopNests.get(1)._loopHead.bid);
        Assertions.assertEquals(5, loopNests.get(2)._loopHead.bid);
        Assertions.assertEquals(8, loopNests.get(3)._loopHead.bid);
