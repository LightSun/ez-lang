...
public class ExitSSABriggs {
    ...
    private void scheduleCopies(BasicBlock block, List<Integer> pushed) {
        ...
        for (BasicBlock s: block.successors) {
            ...
            for (Instruction.Phi phi: s.phis()) {
                ...
                if (srcOperand instanceof Operand.RegisterOperand) {
                    Operand.RegisterOperand srcRegisterOperand = (Operand.RegisterOperand) srcOperand;
        ...
        while (!workList.isEmpty() || !copySet.isEmpty()) {
            while (!workList.isEmpty()) {
                ...
                if (src instanceof Operand.RegisterOperand) {
                    Operand.RegisterOperand srcRegisterOperand = (Operand.RegisterOperand) src;
                ...
                else if (src instanceof Operand.ConstantOperand) {
                    Operand.ConstantOperand srcConstantOperand = (Operand.ConstantOperand) src;
    ...
    private void insertAfterPhi(BasicBlock bb, Register phiDef, Instruction newInst) {
        ...
        for (int pos = 0; pos < bb.instructions.size(); pos++) {
            ...
            if (i instanceof Instruction.Phi) {
                Instruction.Phi phi = (Instruction.Phi) i;
    ...
    private Register addMoveToTempAtBBEnd(BasicBlock block, Register dest) {
        Register temp = function.registerPool.newTempReg(dest.name(), dest.type);
        Instruction.Move inst = new Instruction.Move(new Operand.RegisterOperand(dest), new Operand.RegisterOperand(temp));
    ...
    private void addMoveAtBBEnd(BasicBlock block, Register src, Register dest) {
        Instruction.Move inst = new Instruction.Move(new Operand.RegisterOperand(src), new Operand.RegisterOperand(dest));
        insertAtEnd(block, inst);
        ...
        Instruction brInst = new List2<>(block.instructions).getLast();
        if (brInst instanceof Instruction.ConditionalBranch) {
            Instruction.ConditionalBranch cbr = (Instruction.ConditionalBranch) brInst;
    ...
    private void addMoveAtBBEnd(BasicBlock block, Operand.ConstantOperand src, Register dest) {
        Instruction.Move inst = new Instruction.Move(src, new Operand.RegisterOperand(dest));
        insertAtEnd(block, inst);
    }
    ...
    static class NameStack {
        ...
        Register top() { return new List2<>(stack).getLast(); }
        void pop() { new List2<>(stack).removeLast(); }
