...
public abstract class Instruction {
    ...
    public List<Register> uses() {
        ...
        for (int i = 0; i < uses.length; i++) {
            ...
            if (operand != null && operand instanceof Operand.RegisterOperand) {
    ...
    public void replaceUses(Register[] newUses) {
        ...
        for (int i = 0; i < uses.length; i++) {
            Operand use = uses[i];
            if (use != null && use instanceof Operand.RegisterOperand) {
                Operand.RegisterOperand registerOperand = (Operand.RegisterOperand) use;
    ...
    public boolean replaceUse(Register source, Register target) {
        ...
        for (int i = 0; i < uses.length; i++) {
            Operand operand = uses[i];
            if (operand != null && operand instanceof Operand.RegisterOperand
                    && ((Operand.RegisterOperand)operand).reg.id == source.id) {
                Operand.RegisterOperand registerOperand = (Operand.RegisterOperand) operand;
    ...
    public void replaceUseWithConstant(Register register, Operand.ConstantOperand constantOperand) {
        for (int i = 0; i < uses.length; i++) {
            Operand operand = uses[i];
            if (operand != null && operand instanceof Operand.RegisterOperand
                    && ((Operand.RegisterOperand)operand).reg.id == register.id) {
    ...
    public static class Phi extends Instruction {
        ...
        public void removeInput(int i) {
            Operand[] newUses = new Operand[uses.length - 1];
