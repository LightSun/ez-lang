...
public class ConstantComparisonPropagation {
    ...
    private void propagateConstantsInComparisons(BasicBlock block) {
        ...
        Instruction instruction = new List2<>(block.instructions).getLast();
        if (instruction instanceof Instruction.ConditionalBranch) {
            ...
            if (cbr.condition() instanceof Operand.RegisterOperand) {
                Operand.RegisterOperand conditionVar = (Operand.RegisterOperand) cbr.condition();
                ...
                if (defUse.instruction.block.bid == block.bid
                        && defUse.instruction instanceof Instruction.Binary
                        && ((Instruction.Binary)defUse.instruction).binOp.equals("==")) {
                    Instruction.Binary binary = (Instruction.Binary) defUse.instruction;
                    // Get the constant and the register operands
                    // from the binary
                    Operand.ConstantOperand constantOp = null;
                    Operand.RegisterOperand registerOp = null;
                    if (binary.left() instanceof Operand.ConstantOperand
                            && binary.right() instanceof Operand.RegisterOperand) {
                        Operand.ConstantOperand leftConstant = (Operand.ConstantOperand) binary.left();
                        Operand.RegisterOperand rightReg = (Operand.RegisterOperand) binary.right();
                        constantOp = leftConstant;
                        registerOp = rightReg;
                    } else if (binary.right() instanceof Operand.ConstantOperand
                            && binary.left() instanceof Operand.RegisterOperand) {
                        Operand.ConstantOperand rightConstant = (Operand.ConstantOperand) binary.right();
                        Operand.RegisterOperand leftReg = (Operand.RegisterOperand) binary.left();
                    ...
                    if (constantOp != null) {
                        ...
                        if (!checkUsedInPhi(trueBlock, registerOp.reg)) {
                            ...
                            // be adding in future
                            Register replacementRegister = function.registerPool.newTempReg(registerOp.reg.type);
                            Instruction.Move defInst = new Instruction.Move(constantOp, new Operand.TempRegisterOperand(replacementRegister));
                            insertAtBeginning(trueBlock, defInst);
                            SSAEdges.SSADef replacementRegisterUses = SSAEdges.addDef(ssaDefUse, replacementRegister, defInst); // Update SSA Def Use chains, add def for new reg
    ...
    private static boolean checkUsedInPhi(BasicBlock block, Register register) {
        for (Instruction instruction : block.instructions) {
            if (instruction instanceof Instruction.Phi) {
                Instruction.Phi phi = (Instruction.Phi) instruction;
