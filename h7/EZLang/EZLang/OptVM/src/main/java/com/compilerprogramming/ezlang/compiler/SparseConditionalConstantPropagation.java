...
public class SparseConditionalConstantPropagation {
    ...
    private void visitBlock(BasicBlock b) {
        for (Instruction.Phi phi : b.phis()) {
    ...
    private void visitInstruction(Instruction instruction) {
        ...
        if (evalInstruction(instruction)) {
            if (instruction instanceof Instruction.ConditionalBranch || instruction instanceof Instruction.Jump) {
                ...
                Register def = instruction instanceof Instruction.Phi ?
                        ((Instruction.Phi)instruction).value() : instruction.def();
    ...
    private void removeBranchesThatAreNotExecutable() {
        for (FlowEdge flowEdge : flowEdges.keySet()) {
    ...
    private void removeEdge(BasicBlock source, BasicBlock target) {
        ...
        if (instruction instanceof Instruction.ConditionalBranch) {
            Instruction.ConditionalBranch cbr = (Instruction.ConditionalBranch) instruction;
        ...
        for (Instruction.Phi phi: target.phis()) {
            phi.removeInput(j);
        }
        // update cfg
    ...
    private void replaceVarsWithConstants() {
        for (Register register: valueLattice.getRegisters()) {
            LatticeElement latticeElement = valueLattice.get(register);
            if (latticeElement.kind == V_CONSTANT) {
                Operand.ConstantOperand constant = new Operand.ConstantOperand(latticeElement.value, register.type);
                SSAEdges.SSADef defUseChain = this.ssaEdges.get(register);
                // replace uses with constant
                for (Instruction usingInstruction: defUseChain.useList) {
                ...
                BasicBlock block = defUseChain.instruction.block;
    ...
    private boolean evalInstruction(Instruction instruction) {
        ...
        if(instruction instanceof Instruction.Ret){
            ...
            if (moveInst.to() instanceof Operand.RegisterOperand) {
                ...
                LatticeElement cell = valueLattice.get(toReg.reg);
            ...
            if (cbrInst.condition() instanceof Operand.RegisterOperand) {
                ...
                LatticeElement cell = valueLattice.get(registerOperand.reg);
            ...
            if (!(callInst.callee.returnType instanceof EZType.EZTypeVoid)) {
                LatticeElement cell = valueLattice.get(callInst.returnOperand().reg);
                changed = cell.setKind(V_VARYING);
            }
        } //Instruction.Unary unaryInst
            ...
            LatticeElement cell = valueLattice.get(unaryInst.result().reg);
            LatticeElement input = valueLattice.get(unaryOperand.reg);
            ...
            LatticeElement cell = valueLattice.get(binaryInst.result().reg);
            ...
            LatticeElement cell = valueLattice.get(newArrayInst.destOperand().reg);
            changed = cell.setKind(V_VARYING);
            ...
            LatticeElement cell = valueLattice.get(newStructInst.destOperand().reg);
            changed = cell.setKind(V_VARYING);
            ...
            LatticeElement cell = valueLattice.get(arrayLoadInst.destOperand().reg);
            changed = cell.setKind(V_VARYING);
            ...
            LatticeElement cell = valueLattice.get(getFieldInst.destOperand().reg);
            changed = cell.setKind(V_VARYING);
        }else if(instruction instanceof Instruction.ArgInstruction){
            Instruction.ArgInstruction argInst = (Instruction.ArgInstruction) instruction;
            LatticeElement cell = valueLattice.get(argInst.def());
            changed = cell.setKind(V_VARYING);
        }else if(instruction instanceof Instruction.Phi){
            Instruction.Phi phiInst = (Instruction.Phi) instruction;
...
//        }
    ...
    private boolean visitPhi(BasicBlock block, Instruction.Phi phiInst) {
        ...
        for (int j = 0; j < block.predecessors.size(); j++) {
            ...
            if (isEdgeExecutable(pred, block)) {
                ...
                else if (phiInst.input(j) instanceof Operand.ConstantOperand) {
                    Operand.ConstantOperand constantOperand = (Operand.ConstantOperand) phiInst.input(j);
    ...
    private boolean markEdgeExecutable(BasicBlock source, BasicBlock target) {
        FlowEdge edge = new FlowEdge(source, target);
        Boolean oldValue = flowEdges.get(edge);
    ...
    static final class ValueLattice {
        ...
        LatticeElement get(Register reg) {
            LatticeElement cell = valueLattice.get(reg);
            if (cell == null) {
    ...
    static final class WorkList<E> {
        ...
        E pop() {
            ...
            E x = new List2<>(list).removeFirst();
            members.remove(x);
            return x;
        }
