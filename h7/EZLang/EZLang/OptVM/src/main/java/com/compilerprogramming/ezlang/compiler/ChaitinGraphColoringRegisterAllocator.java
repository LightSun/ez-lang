...
public class ChaitinGraphColoringRegisterAllocator {
    ...
    public Map<Integer, Integer> assignRegisters(CompiledFunction function, int numRegisters, EnumSet<Options> options) {
        ...
        // Create color set
        List<Integer> colors = new ArrayList<>(IntStream.range(0, numRegisters).boxed().collect(Collectors.toList()));
    ...
    private Map<Integer, Integer> preAssignArgsToColors(CompiledFunction function, Set<Integer> registers, List<Integer> colors) {
        ...
        for (Instruction instruction : function.entry.instructions) {
            if (instruction instanceof Instruction.ArgInstruction) {
                Instruction.ArgInstruction argInstruction = (Instruction.ArgInstruction) instruction;
        ...
        for (BasicBlock block: function.getBlocks()) {
            ...
            while (iter.hasNext()) {
                ...
                if (instruction instanceof Instruction.Move
                    && ((Instruction.Move)instruction).from() instanceof Operand.RegisterOperand) {
                    Instruction.Move move = ((Instruction.Move)instruction);
                    Operand.RegisterOperand registerTarget = (Operand.RegisterOperand) ((Instruction.Move)instruction).from();
    ...
    private Set<Integer> getNeighborColors(InterferenceGraph g, Integer node, Map<Integer,Integer> assignedColors) {
        ...
        for (Integer neighbour: g.neighbors(node)) {
    ...
    private Integer chooseSomeColorNotAssignedToNeighbors(Set<Integer> colors, Set<Integer> neighborColors) {
        ...
        HashSet<Integer> set = new HashSet<>(colors);
    ...
    private static HashSet<Integer> subtract(Set<Integer> originalSet, Integer node) {
        HashSet<Integer> reducedSet = new HashSet<>(originalSet);
