...
public class EnterSSA {
    ...
    private void findNonLocalNames() {
        for (BasicBlock block : blocks) {
            HashSet<Integer> varKill = new HashSet<Integer>();
    ...
    void insertPhis() {
        for (int i = 0; i < nonLocalNames.length; i++) {
            ...
            if (x != null) {
                ...
                WorkList worklist = new WorkList(blockSets[x.nonSSAId()].blocks);
                BasicBlock b = worklist.pop();
                while (b != null) {
                    visited.set(b.bid);
    ...
    Register makeVersion(Register reg) {
        ...
        Register.SSARegister ssaReg = function.registerPool.ssaReg(reg, version);
    ...
    void search(BasicBlock block) {
        ...
        for (Instruction i: block.instructions) {
            ...
            if (i.definesVar() || i instanceof Instruction.Phi) {
                Register reg = i instanceof Instruction.Phi ? ((Instruction.Phi)i).value() : i.def();
    ...
    static class VersionStack {
        ...
        Register.SSARegister top() {
            ...
            return new List2<>(stack).getLast();
        }
    ...
    public static class WorkList {
        ...
        BasicBlock pop() {
            if ( blocks.isEmpty() )
                return null;
            BasicBlock x = new List2<>(blocks).removeFirst();
            members.clear(x.bid);
            return x;
        }
    }
