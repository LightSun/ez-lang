...
import com.compilerprogramming.ezlang.utils.List2;
...
public class CompiledFunction {
    ...
    private class ExprImpl implements IExprCompiler{

        @Override
        public void compileConstantExpr(AST.LiteralExpr expr) {
            CompiledFunction.this.compileConstantExpr(expr);
        }
        @Override
        public void compileBinaryExpr(AST.BinaryExpr expr) {
            CompiledFunction.this.compileBinaryExpr(expr);
        }

        @Override
        public void compileUnaryExpr(AST.UnaryExpr expr) {
            CompiledFunction.this.compileUnaryExpr(expr);
        }

        @Override
        public void compileSymbolExpr(AST.NameExpr expr) {
            CompiledFunction.this.compileSymbolExpr(expr);
        }

        @Override
        public void compileNewExpr(AST.NewExpr expr) {
            CompiledFunction.this.compileNewExpr(expr);
        }

        @Override
        public void compileInitExpr(AST.InitExpr expr) {
            CompiledFunction.this.compileInitExpr(expr);
        }

        @Override
        public void compileArrayIndexExpr(AST.ArrayLoadExpr expr) {
            CompiledFunction.this.compileArrayIndexExpr(expr);
        }

        @Override
        public void compileArrayStoreExpr(AST.ArrayStoreExpr expr) {
            CompiledFunction.this.compileArrayStoreExpr(expr);
        }

        @Override
        public void compileFieldExpr(AST.GetFieldExpr expr) {
            CompiledFunction.this.compileFieldExpr(expr);
        }

        @Override
        public void compileSetFieldExpr(AST.SetFieldExpr expr) {
            CompiledFunction.this.compileSetFieldExpr(expr);
        }
        @Override
        public void compileCallExpr(AST.CallExpr expr) {
            CompiledFunction.this.compileCallExpr(expr);
        }
    }
    ...
    private void generateArgInstructions(Scope scope) {
        if (scope.isFunctionParameterScope) {
            for (Symbol symbol: scope.getLocalSymbols()) {
                if (symbol instanceof Symbol.ParameterSymbol) {
                    Symbol.ParameterSymbol parameterSymbol = (Symbol.ParameterSymbol) symbol;
    ...
    private boolean isBlockTerminated(BasicBlock block) {
        return (block.instructions.size() > 0 &&
                new List2<>(block.instructions).getLast().isTerminal());
    }
    ...
    private boolean compileExpr(AST.Expr expr) {
        CompileHelper.compileExpr(expr, new ExprImpl());
        return true;
    }
    ...
    private boolean compileCallExpr(AST.CallExpr callExpr) {
        ...
        if (callee instanceof Operand.LocalFunctionOperand) {
            Operand.LocalFunctionOperand functionOperand = (Operand.LocalFunctionOperand) callee;
        ...
        int returnStackPos = virtualStack.size();
        ...
        if (callExpr.callee.type instanceof EZType.EZTypeFunction &&
                !(((EZType.EZTypeFunction)callExpr.callee.type).returnType instanceof EZType.EZTypeVoid)) {
            ret = createTemp(((EZType.EZTypeFunction)callExpr.callee.type).returnType);
        }
    ...
    private EZType.EZTypeStruct getStructType(EZType t) {
        return CompileHelper.getStructType(t);
    }
    ...
    private boolean compileSymbolExpr(AST.NameExpr symbolExpr) {
        if (symbolExpr.type instanceof EZType.EZTypeFunction){
            EZType.EZTypeFunction functionType = (EZType.EZTypeFunction) symbolExpr.type;
    ...
    private boolean codeBoolean(AST.BinaryExpr binaryExpr) {
        ...
        Operand.RegisterOperand temp = ensureTemp();    // Normally temps are SSA but this temp gets two assignments, thus must be included during SSA conversion
        jumpTo(l3);
    ...
    private boolean compileBinaryExpr(AST.BinaryExpr binaryExpr) {
        ...
        if (left instanceof Operand.NullConstantOperand &&
        ...
        else if (left instanceof Operand.ConstantOperand &&
                right instanceof Operand.ConstantOperand ) {
            Operand.ConstantOperand leftconstant = (Operand.ConstantOperand) left;
            Operand.ConstantOperand rightconstant = (Operand.ConstantOperand) right;
            long value = 0;
            switch (opCode) {
            ...
            Operand.RegisterOperand temp = createTemp(binaryExpr.type);
    ...
    private boolean compileUnaryExpr(AST.UnaryExpr unaryExpr) {
        ...
        if (top instanceof Operand.ConstantOperand) {
            Operand.ConstantOperand constant = (Operand.ConstantOperand) top;
            switch (opCode) {
            ...
            Operand.RegisterOperand temp = createTemp(unaryExpr.type);
    ...
    private Operand.TempRegisterOperand createTemp(EZType type) {
        Operand.TempRegisterOperand tempRegister = new Operand.TempRegisterOperand(registerPool.newTempReg(type));
        pushOperand(tempRegister);
        return tempRegister;
    }

    EZType typeOfOperand(Operand operand) {
        if (operand instanceof Operand.ConstantOperand) {
            Operand.ConstantOperand constant = (Operand.ConstantOperand) operand;
            return constant.type;
        }
        else if (operand instanceof Operand.NullConstantOperand) {
            Operand.NullConstantOperand nullConstantOperand = (Operand.NullConstantOperand) operand;
            return nullConstantOperand.type;
        }
        else if (operand instanceof Operand.RegisterOperand) {
    ...
    private Operand.TempRegisterOperand createTempAndMove(Operand src) {
        ...
        Operand.TempRegisterOperand temp = createTemp(type);
        codeMove(src, temp);
        return temp;
    }
    ...
    private Operand.RegisterOperand ensureTemp() {
        ...
        } else if (top instanceof Operand.TempRegisterOperand) {
            return (Operand.RegisterOperand) top;
    ...
    private Operand pop() {
        return new List2<>(virtualStack).removeLast();
    }

    private Operand top() {
        return new List2<>(virtualStack).getLast();
    }
    ...
    private Operand.TempRegisterOperand codeIndexedLoad() {
        Operand indexed = pop();
        Operand.TempRegisterOperand temp = createTemp(indexed.type);
        if (indexed instanceof Operand.LoadIndexedOperand) {
            Operand.LoadIndexedOperand loadIndexedOperand = (Operand.LoadIndexedOperand) indexed;
            codeArrayLoad(loadIndexedOperand, temp);
        }
        else if (indexed instanceof Operand.LoadFieldOperand) {
            Operand.LoadFieldOperand loadFieldOperand = (Operand.LoadFieldOperand) indexed;
    ...
    private void codeNew(EZType type, AST.Expr len, AST.Expr initVal) {
        if (type instanceof EZType.EZTypeArray)
            codeNewArray((EZType.EZTypeArray) type, len, initVal);
        else if (type instanceof EZType.EZTypeStruct) {
            codeNewStruct((EZType.EZTypeStruct) type);
        }
        else
    ...
    private void codeNewArray(EZType.EZTypeArray typeArray, AST.Expr len, AST.Expr initVal) {
        Operand.TempRegisterOperand temp = createTemp(typeArray);
        ...
        Instruction insn;
        Operand.RegisterOperand target = (Operand.RegisterOperand) issa.write(temp);
        if (lenOperand != null) {
    ...
    private void codeNewStruct(EZType.EZTypeStruct typeStruct) {
        Operand.TempRegisterOperand temp = createTemp(typeStruct);
        Operand.RegisterOperand target = (Operand.RegisterOperand) issa.write(temp);
        Instruction.NewStruct insn = new Instruction.NewStruct(typeStruct, target);
        issa.recordDef(target, insn);
        code(insn);
    }

    private void codeArg(Operand.LocalRegisterOperand target) {
        Operand.RegisterOperand newtarget = (Operand.RegisterOperand) issa.write(target);
        Instruction.ArgInstruction insn = new Instruction.ArgInstruction(newtarget);
    ...
    private void codeMove(Operand srcOperand, Operand destOperand) {
        ...
        Instruction.Move insn = new Instruction.Move(srcOperand, destOperand);
    ...
    private void codeReturn(Operand resultOperand) {
        ...
        Instruction.Ret insn = new Instruction.Ret(resultOperand);
    ...
    private void codeCBR(BasicBlock block, Operand condition, BasicBlock trueBlock, BasicBlock falseBlock) {
        ...
        Instruction.ConditionalBranch insn = new Instruction.ConditionalBranch(block, condition, trueBlock, falseBlock);
    ...
    private void codeCall(int newBase,
        ...
        Instruction.Call insn = new Instruction.Call(newBase, targetOperand, calleeType, args);
        for (int i = 0; i < args.length; i++) {
    ...
    private void codeUnary(String opCode, Operand.RegisterOperand result, Operand operand) {
        ...
        Instruction.Unary insn = new Instruction.Unary(opCode, result, operand);
    ...
    private void codeBinary(String opCode, Operand.RegisterOperand result, Operand left, Operand right) {
        ...
        result = (Operand.RegisterOperand) issa.write(result);
        Instruction.Binary insn = new Instruction.Binary(opCode, result, left, right);
        issa.recordDef(result, insn);
    ...
    private void codeArrayLoad(Operand.LoadIndexedOperand loadIndexedOperand, Operand.RegisterOperand target) {
        ...
        Instruction.ArrayLoad insn = new Instruction.ArrayLoad(loadIndexedOperand, target);
        issa.recordDef(target, insn);
    ...
    private void codeGetField(Operand.LoadFieldOperand loadFieldOperand, Operand.RegisterOperand target) {
        ...
        target = (Operand.RegisterOperand) issa.write(target);
        Instruction.GetField insn = new Instruction.GetField(loadFieldOperand, target);
        issa.recordDef(target, insn);
    ...
    private void codeArrayStore(Operand value, Operand.LoadIndexedOperand loadIndexedOperand) {
        ...
        Instruction.ArrayStore insn = new Instruction.ArrayStore(value, loadIndexedOperand);
        issa.recordUse(loadIndexedOperand.arrayOperand, insn);
        issa.recordUse(loadIndexedOperand.indexOperand, insn);
    ...
    private void codeSetField(Operand value, Operand.LoadFieldOperand loadFieldOperand) {
        loadFieldOperand = new Operand.LoadFieldOperand(issa.read(loadFieldOperand.structOperand), loadFieldOperand.fieldName, loadFieldOperand.fieldIndex);
        value = issa.read(value);
        Instruction.SetField insn = new Instruction.SetField(value, loadFieldOperand);
        issa.recordUse(loadFieldOperand.structOperand, insn);
        issa.recordUse(value, insn);
        code(insn);
    }
    ...
    static final class IncrementalSSABraun implements IncrementalSSA {
        ...
        private Register tryRemovingPhi(Instruction.Phi phi) {
            ...
            List<Instruction> users = getUsesExcept(phi);
            ...
            for (Instruction use: users) {
                if (use instanceof Instruction.Phi)
                    tryRemovingPhi((Instruction.Phi) use);
            }
            return same;
        }
        ...
        private void replacePhiValueAndUsers(Instruction.Phi phi, Register newValue) {
            Register oldValue = phi.value();
            SSAEdges.SSADef oldDefUseChain = ssaDefUses.get(oldValue);
            SSAEdges.SSADef newDefUseChain = ssaDefUses.get(newValue);
            ...
            if (oldDefUseChain != null) {
                for (Instruction instruction: oldDefUseChain.useList) {
                    ...
                    if (instruction instanceof Instruction.Phi) {
                        Instruction.Phi somePhi = (Instruction.Phi) instruction;
        ...
        private List<Instruction> getUsesExcept(Instruction.Phi phi) {
            SSAEdges.SSADef oldDefUseChain = ssaDefUses.get(phi.value());
            if (oldDefUseChain == null) {
                return new ArrayList<>();
            }
            List<Instruction> useList = new ArrayList<>(oldDefUseChain.useList);
            useList.remove(phi);
            return useList;
        }
        ...
        private void replaceDefs(Register oldValue, Register newValue) {
            Map<BasicBlock, Register> defs = currentDef.get(oldValue.nonSSAId());
            for (BasicBlock bb: defs.keySet())
        ...
        public Operand read(Operand operand) {
            ...
            if (operand instanceof Operand.RegisterOperand) {
                Operand.RegisterOperand localRegisterOperand = (Operand.RegisterOperand) operand;
                Register reg = readVariable(localRegisterOperand.reg, function.currentBlock);
        ...
        public Operand write(Operand operand) {
            ...
            if (operand instanceof Operand.RegisterOperand) {
                Operand.RegisterOperand localRegisterOperand = (Operand.RegisterOperand) operand;
                Register variable = localRegisterOperand.reg;
        ...
        public void sealBlock(BasicBlock block) {
            if (isSealed(block))
                return;
            Map<Register, Instruction.Phi> pendingPhis = incompletePhis.remove(block);
            if (pendingPhis != null) {
                for (Register variable : pendingPhis.keySet()) {
